@if($featuredProducts->count() > 0)
    <div class="mb-12">
                ...
                <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 hover:shadow-md transition-shadow duration-300">
                    <a href="{{ route('products.show', $product->slug) }}" class="block">
                        <div class="h-48 bg-gray-100 overflow-hidden">
                            <img src="{{ $product->mainImage && $product->mainImage->image_path ? Storage::url($product->mainImage->image_path) : asset('images/no-image.png') }}" 
        ...
        <div class="text-center mt-8">
            <a href="{{ route('products.featured') }}" class="inline-block text-indigo-600 font-semibold hover:text-indigo-800 hover:underline">
                すべてのおすすめ商品を見る →
            </a>
        </div>
    </div>
